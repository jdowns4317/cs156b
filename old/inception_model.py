...
def get_output(train_loader, test_loader):
    ...
    model.fc = nn.Linear(num_ftrs, 3)  # New classifier for 3 classes
...
submission_df.to_csv('results/inception_submission.csv', index=False)

probs_df = pd.DataFrame(probs_dict_final)
probs_df = probs_df.sort_values(by = "Id")
probs_df.to_csv('results/inception_probs_submission.csv', index=False)
